...
PreIntegration::PreIntegration(Vector3d acc_0_,Vector3d acc_1_,Vector3d gyr_0_,Vector3d gyr_1_,Vector3d ba_,Vector3d bg_,double dt_):
        dt(dt_),sum_t(0.0),acc_0(acc_0_),gyr_0(gyr_0_),acc_first(acc_0_),gyr_first(gyr_0_),acc_1(acc_1_),gyr_1(gyr_1_),ba_pre(ba_),bg_pre(bg_),jacobian(Eigen::Matrix<double,15,15>::Identity()),
...
void PreIntegration::run()
{
    ...
    //中值法
    dq_1=dq*Eigen::Quaterniond(1,0.5*gyr_aver(0)*dt,0.5*gyr_aver(1)*dt,0.5*gyr_aver(2)*dt);
